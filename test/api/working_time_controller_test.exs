defmodule Todolist.WorkingTimeControllerTest do
  use Todolist.DataCase

  alias Todolist.WorkingTimeController

  describe "workingtimes" do
    alias Todolist.WorkingTimeController.WorkingTime

    import Todolist.WorkingTimeControllerFixtures
    ...
    test "create_working_time/1 with valid data creates a working_time" do
      valid_attrs = %{end: ~N[2021-10-24 14:54:00], start: ~N[2021-10-24 14:54:00]}

      assert {:ok, %WorkingTime{} = working_time} = WorkingTimeController.create_working_time(valid_attrs)
      assert working_time.end == ~N[2021-10-24 14:54:00]
      assert working_time.start == ~N[2021-10-24 14:54:00]
    end
    ...
    test "update_working_time/2 with valid data updates the working_time" do
      working_time = working_time_fixture()
      update_attrs = %{end: ~N[2021-10-25 14:54:00], start: ~N[2021-10-25 14:54:00]}

      assert {:ok, %WorkingTime{} = working_time} = WorkingTimeController.update_working_time(working_time, update_attrs)
      assert working_time.end == ~N[2021-10-25 14:54:00]
      assert working_time.start == ~N[2021-10-25 14:54:00]
    end
